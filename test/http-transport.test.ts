...
// `bin/vgi-etf-jpmorgan-worker` and talks to it over stdin/stdout; the haybarn suite exercises that).
...
//   - the function + products/holdings tables are exposed over HTTP  [network-free]
//   - a full products scan round-trips over HTTP                    [live: J.P. Morgan]
//
// The final scan hits J.P. Morgan live (like the haybarn live-invariant asserts) — fine for an
...
test("catalog is discoverable over HTTP", async () => {
  ...
  try {
    ...
    expect(await client.catalogs()).toContain("jpmorgan");
  } finally {
    rpc.close();
  }
});

test("the function and the products/holdings tables are exposed over HTTP", async () => {
  const rpc = httpConnect(baseUrl, { prefix: PREFIX });
  try {
    const client = new VgiClient(rpc);
    const attach = await client.catalogAttach("jpmorgan");
    const fns = await client.schemaContentsFunctions(attach.attach_opaque_data, "main", "TABLE_FUNCTION");
    // The holdings backing scan is listed under the SAME name as the `holdings` table it backs
    // (required so the extension pushes the ticker filter into the table); products' backing scan
    // stays unlisted.
...
test("products table scan round-trips over HTTP (live)", async () => {
  ...
  try {
    ...
    // Live: J.P. Morgan lists ~75 US ETFs.
    ...
    // Live invariant: JEPI is in the ETF list.
    expect(rows.some((r) => r.ticker === "JEPI")).toBe(true);
  } finally {
    rpc.close();
  }
});
